...
```bash
export GRAFANA_ROOT_URL=https://<your-tailscale-serve-name>/grafana/
docker compose --profile monitoring-lite --profile monitoring-full up -d
```

Set `GRAFANA_ROOT_URL` in the environment or `.env` to the exact Tailscale Serve HTTPS URL ending in `/grafana/`. Do not use only the bare host name, because Grafana generates absolute redirects and asset URLs from this value.
